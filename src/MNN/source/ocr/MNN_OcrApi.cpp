...
/* DLL 导出宏 */
#if defined(_MSC_VER)
#define MNN_OCR_API __declspec(dllexport)
#else
#define MNN_OCR_API __attribute__((visibility("default")))
#endif
...
MNN_OCR_API MNN_OCRResult* MNN_ocrRecognize(const uint8_t* image_data, int width, int height) {
    ...
    if (!engine || !image_data || width <= 0 || height <= 0) return nullptr;

    auto text_lines = engine->recognize(image_data, width, height);

    auto* result = new MNN_OCRResult();
...
MNN_OCR_API MNN_OCRResult* MNN_ocrRecognizeFile(const char* image_path) {
    ...
    auto* result = MNN_ocrRecognize(img_data, width, height);
